...
import { addToCategory, deleteCategory, getAVideo, getAllCategories, updateCategory } from './services/allAPI'
   ...
   </div>
{
  allCategories?allCategories?.map(item=>(
    <div className='mt-5 mb-3 border rounded p-3' style={{color:'white'}}   droppable onDragOver={(e)=>onDragOver(e)} onDrop={(e)=>videoDrop(e,item?.id)}>
              <div className='  d-flex justify-content-between align-items-center '>
              <h6 className='text-light'>{item?.categoryname}</h6>
              ...
              <Row>
                {
                  ...
                  item?.allVideos.map(card=>(
                    <Col>
                    <Videocart displayData={card} insideCategory={true} />
                    </Col>
                  ))
                }
              </Row>
